...
to run type
cd downloads && python hps_tuner.py
into command prompt
'''
...
WHITE_NOISE_THRESH = 0.3 # everything under WHITE_NOISE_THRESH*avg_energy_per_freq is cut off
...
def callback(indata, frames, time, status):
  ...
  if any(indata):
    ...
    #Keyboard Inputs LETSSS GOOOOOO (made by Amo)

    if closest_note.startswith('A#'):
       print("click")
       pyautogui.click()
    elif closest_note.startswith('A'):
        print("W")
        keyboard.press('w')
        keyboard.call_later(keyboard.release, args=('w'), delay=1.75)

    if closest_note.startswith('B'):
       print("D")
       keyboard.press('d')
       keyboard.call_later(keyboard.release, args=('d'), delay=1.25)

    if closest_note.startswith('C#'):
       print("space")
       keyboard.send('space', do_press=True, do_release=True)
    elif closest_note.startswith('C'):
      print("A")
      keyboard.press('a')
      keyboard.call_later(keyboard.release, args=('a'), delay=1.25)

    if closest_note.startswith('D#'):
       print("Q")
       keyboard.press('q')
       keyboard.call_later(keyboard.release, args=('q'), delay=0.5)
    elif closest_note.startswith('D'):
      print("E")
      keyboard.press('e')
      keyboard.call_later(keyboard.release, args=('e'), delay=0.5)
    
    if closest_note.startswith('E'):
       print("right click")
       pyautogui.click(button='right')

    if closest_note.startswith('F#'):
       print("shift+w")
       keyboard.press('w')
       keyboard.press('shift')
       keyboard.call_later(keyboard.release, args=('w'), delay=2)
       keyboard.call_later(keyboard.release, args=('shift'), delay=2)
    elif closest_note.startswith('F'):
       print("S")
       keyboard.press('s')
       keyboard.call_later(keyboard.release, args=('s'), delay=1.25)

    if closest_note.startswith('G#'):
       print("scroll up")
       pyautogui.scroll(200)
    elif closest_note.startswith('G'):
      print("scroll down")
      pyautogui.scroll(-200)
try:
